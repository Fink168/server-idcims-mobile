		...
		<block v-for="(page,page_index) in subData" :key="page_index">
			<view class="sub-menu-class" :class="{'show':showPage==page_index,'hide':pageState[page_index]!=true}">
				<block v-if="page.type=='hierarchy'">
					<scroll-view class="sub-menu-list" :class="[page.submenu[activeMenuArr[page_index][0]].submenu.length>0?'first':'alone']"
						 ...
						 :scroll-into-view="'second_id'+secondScrollInto">
							<block v-for="(sub_second,second_index) in sub.submenu" :key="second_index">
								<view class="sub-menu" :id="'second_id'+second_index" :class="{'on':activeMenuArr[page_index][1]==second_index}">
									...
									<view class="more-sub-menu" v-if="sub.submenu.length>0&&sub_second.submenu.length>0">
...
<script>
	export default {
		...
		watch: {
			filterData(newVal) {
				this.initMenu(); //filterData变化重新初始化菜单
			},
		...
		methods: {
			initMenu() {
				...
				//初始化选中项数组
				tmpMenuActiveArr = this.activeMenuArr.length>0?this.activeMenuArr:this.defaultSelected.length>0?this.defaultSelected:tmpMenuActiveArr;
			...
			selectHierarchyMenu(page_index, level1_index, level2_index, level3_index) {
				...
				if (level1_index != null && level2_index == null && level3_index == null && this.shadowActiveMenuArr[page_index][0] ==
					...
					level2_index!=null&&this.activeMenuArr[page_index].splice(1, 1, level2_index);
					level3_index!=null&&this.activeMenuArr[page_index].splice(2, 1, level3_index);
				}
				//写入结果
				if (level3_index != null ||
					level2_index != null ||
