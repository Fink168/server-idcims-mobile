<template name="graceui-alert">
	<view :class="[show ? 'ganimate-show grace-alert' : 'grace-alert', 'm-'+msgtype]" :hidden="!show">
		<view class="icon gaicon" :class="['icon-'+msgtype, msgtype]"></view>
		<view class="grace-alert-msg" :class="[msgtype]">{{msg}}</view>
	</view>
</template>
<script>
	export default {
		name: "grace-alert",
		props: {
			msgtype:{
				type : String,
				default : "info"
			},
			msg: {
				type: String,
				default: ""
			},
			show:{
				type : Boolean,
				default : false
			}
		}
	}
</script>
<style>
@font-face {font-family: "gafont";
  src:url('data:application/x-font-woff;charset=utf-8;base64,');
}
.gaicon {
  font-family:"gafont" !important;
  font-size:22px;
  font-style:normal;
}
@keyframes g-show{from {top:-50px;} to {top:0;}}
.ganimate-show{animation: g-show 300ms linear forwards;}
.icon-error:before { content: "\e604";}
.icon-success:before { content: "\e60f";}
.icon-info:before { content: "\e623";}
.grace-alert{display:flex; width:100%; padding:10px 5%; flex-wrap:nowrap; position:fixed; z-index:99999; left:0; top:-60px; box-shadow:5px 3px 3px #F7F8F9;}
.grace-alert .success{color: #67c23a !important;}
.grace-alert .error{color: #f56c6c !important;}
.grace-alert .warning{color: #e6a23c !important;}
.grace-alert .info{color: #909399 !important;}
.grace-alert .icon{width:25px; line-height:30px; justify-content:center; flex-shrink:0; margin-right:12px;}
.grace-alert-msg{line-height:30px; font-size:16px; overflow:hidden; justify-content:flex-start; width:100%; white-space:nowrap; text-overflow:ellipsis;}
.m-success{
	background-color: #f0f9eb;
	border-color: #e1f3d8;
}
.m-warning{
	background-color: #f0f9eb;
	border-color: #e1f3d8;
}
.m-info{
	background-color: #edf2fc;
	border-color: #ebeef5;
}
.m-error{
	background-color: #ffeded;
	border-color: #ffdbdb;
}
</style>